    ...
    router.route('/user/profile').get(function(req, res) {
        ...
        //if(!req.user[1].auth != 'auth'){
            ...
            if (Array.isArray(req.user)) {
                ...
                res.render('user_profile.ejs', {user: req.user});
                  if(database.db){
                      database.ReservationModel.loadbyname(paramUsername, function(err, results){
                        if(err){
                          console.error('예약목록 조회 중 에러 발생 : ' + err.stack);
                          res.writeHead('200', {'content-Type':'text/html;charset=utf8'});
                          res.write('<h2>예약 목록 글 조회 중 에러발생</h2>');
