...
var searchrevpost = function(req, res) {
   console.log('revpost 모듈 안에 있는 searchrevpost 호출됨.');
    ...
    var paramPage = req.body.page || req.query.page;
    var paramPerPage = req.body.perPage || req.query.perPage;
    var paramcon = req.body.con || req.query.con;
    console.log('요청 파라미터 : ' + paramPage + ', ' + paramPerPage);
   ...
   if (database.db) {
      ...
      database.ReservationModel.findByUsername(paramcon, function(err, results) {
         if (err) {
                console.error('게시판 글 목록 조회 중 에러 발생 : ' + err.stack);
         ...
         if (results) {
            ...
            database.ReservationModel.count().exec(function(err, count) {
               ...
               req.app.render('revsearch', context, function(err, html) {
                        if (err) {
                            console.error('응답 웹문서 생성 중 에러 발생 : ' + err.stack);
