...
module.exports = function(router, passport) {
  ...
  // 스터디룸 예약
  router.route('/room/reserve/:id/:stime/:etime').get(function(req, res) {
    ...
    if (!req.user) {
      ...
      var paramStime = req.body.stime || req.query.stime || req.params.stime; // 스터디룸 오픈시간
      var paramEtime = req.body.etime || req.query.etime || req.params.etime; // 스터디룸 마감시간
      ID = paramId;
      ST = paramStime;
      ET = paramEtime;
...
router.post('/room/reserve', function(req, res){
  ...
  if (database.db) {
    ...
    if((intStart - intEnd)>0){
      res.writeHead('200', {'Content-Type':'text/html;charset=utf8'});
      res.write('<script>alert( "시작시간과 끝시간 설정이 잘못 되었습니다. 예약시간을 다시 확인해 주세요")</script>');
      res.write('<script>window.location.href="/room/reserve/'+ID+'/'+ST+'/'+ET+'"</script>');
      res.end();
      return;
    }

    else if ((intfStart-intStart)>0 || (intfEnd-intEnd)<0){
      res.writeHead('200', {'Content-Type':'text/html;charset=utf8'});
      res.write('<script>alert( "예약시간 설정이 잘못 되었습니다. 스터디룸 오픈시간과 마감시간을 확인해 주세요")</script>');
      res.write('<script>window.location.href="/room/reserve/'+ID+'/'+ST+'/'+ET+'"</script>');
      res.end();
      return;
    }
